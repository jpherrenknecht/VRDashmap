...
( function () {
    ...
    function updateFleetHTML (rf) {
        if (rf === undefined) {
            ...
            // console.log('ligne 576 rf  '+Object.keys(rf))
            // console.log('ligne 577 rf  '+rf.table)
            // console.log('ligne 577 keys uinfo  '+Object.keys(rf.uinfo))
            sortFriends(rf);
            var fleetHTML =
                '<table>'
                + '<thead class="sticky">'
    ...
    function sortFriendsByCategory (fleet) {
        ...
        //console.log('    ligne 895 fln '+fln)    // c'est une liste des identifiants
    }
   ...
   // modif jph
    ...
    function updatePosition (message, r) {
        ...
        transmissionajax(message)    // modif jph
    ...
    function initializeMap (race) {
        ...
        if (!race.gdiv) {
            ...
            map.fitBounds(bounds);
             ...
             //****************************************************************** */
            // JPH modif pour avoir le popup  latitude  longitude
            var infoWindow = new google.maps.InfoWindow({    maxWidth: 420   });
            google.maps.event.addListener(map, 'click', function (evt) {
            var position=Util.formatPosition(evt.latLng.lat(),evt.latLng.lng())
            var content = "<div >";
            content += "Latitude  : " + evt.latLng.lat().toFixed(6) + "<br/>";
            content += "Longitude : " + evt.latLng.lng().toFixed(6)+ "<br/>";
            content += " soit <b> "+ position +"</b><br>"
            content +=" ["+ evt.latLng.lat().toFixed(6) +","+ evt.latLng.lng().toFixed(6)+"],</div>"  // modif pour faire du copier coller    

            content += " soit <b> "
            content +=" {\"lat\":"+ evt.latLng.lat().toFixed(6) +",\"lon\":"+ evt.latLng.lng().toFixed(6)+"},</div>"  // modif pour faire du copier coller    
            infoWindow.setContent(content);
            infoWindow.setPosition(evt.latLng);
            infoWindow.open(map);
            });
            // fin modif jp pour le popup 
            //******************************************************************* */
    ...
    function callRouter (raceId, userId = currentUserId, auto = false) {
        ...
        else if      (selRouter.value == 'vrouteur')
            {
        callRouterVRouteur(race, userInfo, isMe, auto); }
            
        else if      (selRouter.value == 'local')
        {
        callRouterlocal(race, userInfo, isMe, auto); }
...
function callRouterVRouteur  (race, userInfo, isMe, auto,currentTeam) {

    var host;
    if (selRouter.value == 'vrouteur') {
        host = 'http://vrouteur.ddns.net';
    } else {
        host = 'localhost:5000';
    }
...
var pos = userInfo.pos;
var twa = userInfo.twa;
var tws = userInfo.tws;
var twd = userInfo.twd;
...
if (userInfo.options)
    ...
    console.log('userinfo :',Object.values(userInfo));
    for (const [key, value] of Object.entries(userInfo)) {
        console.log(`${key}: ${value}`);
      }
    
    console.log(userInfo.speedT.sail)
   // console.log ('sail ='+ sailNames[(r.curr.sail % 10)] )
  
    var baseURL = host;
    var url = host+"/patience?urlBeta="+race.url
            + "&lat=" + pos.lat
            + "&lon=" + pos.lon
            + "&userid=" + getUserId(userInfo)
            + "&username="+username
...
*
*
// Anciennement call routeurlocal
*
*
*
*
...
********************************************************************************************************** */
    ...
    function _handleResponseReceived(request, response, postData) {
        ...
        if (eventClass == 'getboatinfos') {
            ...
            //remarque jph
            console.log('ligne 2667 recuperation de la flotte ')
            // ici on peut recuperer les informations sur la flotte 
            var message = JSON.parse(response.body).res;
            //jph  on peut renvoyer vers le serveur par une requete ajax*******************************************************************
            console.log('ligne 2679 message flotte '+Object.keys(message))
...
// Traitement des reponses boatinfo
    ...
    function handleFleet (request, response, postData) {
        if (response) {
            try {
                ...
                var message = JSON.parse(response.body).res;
                //jph
                console.log('ligne 2761 message flotte '+Object.keys(message))
                console.log('ligne 2766 message flotte '+Object.keys(message['1']))
                console.log('ligne 2766 message flotte  pos e 1    '+message['1']['pos']['lat'])   //donne la position pour le bateau1 mais on a toujoours que l'id 
                console.log('ligne 2772 message flotte  pos e 1    '+message['1']['displayName'])
                console.log('ligne 2772 message flotte  pos e 1    '+message['1']['type'])
                console.log('ligne 2772 message flotte  pos e 1    '+message['1']['speed'])
